...
export default function AdminPanel({ onLogout, settings, theme, isAdminViaPassword, adminRole }: { onLogout: () => void, settings: ClubSettings, theme: 'light' | 'dark', isAdminViaPassword?: boolean, adminRole: 'super' | 'sub' | null }) {
  ...
  const handleResetUserPassword = async (email: string) => {
    if (!confirm(`Sahkan untuk menghantar email penetapan semula kata laluan ke ${email}?`)) return;
    try {
      await sendPasswordResetEmail(auth, email);
      alert(`Email penetapan semula telah dihantar ke ${email}`);
    } catch (err: any) {
      alert("Gagal menghantar email penetapan semula: " + err.message);
    }
  };
          ...
          >
            ...
            {usersLoading ? (
              <div className="py-20 flex flex-col items-center justify-center text-slate-400 gap-4">
                ...
                <p className="text-xs font-black uppercase tracking-widest">Memuatkan Pengguna...</p>
              </div>
            ) : appUsers.length === 0 ? (
              <div className="py-20 flex flex-col items-center justify-center text-slate-400 gap-4 opacity-50">
                <Users className="w-12 h-12" />
                <p className="text-xs font-black uppercase tracking-widest">Tiada pengguna berdaftar</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {appUsers
                  .filter(u => u.email.toLowerCase().includes(userSearchTerm.toLowerCase()) || (u.displayName && u.displayName.toLowerCase().includes(userSearchTerm.toLowerCase())))
                  .map((u) => (
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${u.authProvider === 'google' ? 'bg-blue-50 text-blue-500' : 'bg-orange-50 text-orange-500'}`}>
                              {u.authProvider}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {u.password && (
                          <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-slate-200">
                            <div className="flex flex-col">
                              <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Password</span>
                            ...
                            <button 
                              ...
                              className="p-2 hover:bg-slate-100 rounded-lg transition-colors text-slate-400 hover:text-turquoise"
                            >
                        ...
                        <div className="flex gap-2">
                          <button 
                            ...
                            className="flex items-center gap-2 bg-turquoise/10 text-turquoise hover:bg-turquoise hover:text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                            title="Hantar Email Reset"
                          >
